...
- **资产**：物品按「(买价 − 卖价) ÷ 用了几天」算日均成本，也按类别折旧估出现在值多少（能套预设、能手动估）；基金、股票持仓自动拉行情，取不到就手填价格。持仓市值和实物估值（默认按类别）计入净资产。
- **会员权益**：平台 → 会员/卡 → 权益三层，记清每项权益去哪领、每月几次、什么时候到期；一键打卡、看回本进度，续费和到期有提醒（Android 每天一条通知），流水里看到扣费能一点续上。
- **AI 智能导入**：粘贴、截图、网址、从流水四种方式把会员权益和买的东西导进来，先逐条核对再落库，7 天内能整批撤销。从流水识别订阅不用 AI 也能用。
...
| `AI_IMPORT_PER_HOUR` / `URL_FETCH_PER_MIN` | `20` / `10` | AI 导入每人每小时次数、网址导入每人每分钟抓取次数 |
| `URL_FETCH_ALLOW_FAKEIP` | 空 | 服务端的网络走 Clash 这类 fake-ip 代理（域名都解析成 198.18.x.x）时设成 `1`，网址导入才抓得到。代价：开了之后服务端没法核实真实目标地址，只在信得过全家成员时打开，且只有管理员能用网址导入。改了要重启服务（`docker compose up -d`） |
